...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
  ...
  @Query("""
      select t
      from Transaction t
      where (:accountId is null or t.account.id = :accountId or t.toAccount.id = :accountId)
        and (:categoryId  is null or t.category.id = :categoryId)
        and t.date >= :fromDate
        and t.date <= :toDate
      order by t.date desc
  ...
  /**
   * Returns a ranked list of unique transaction descriptions that start with the
   * given query string.
   *
   * <p>
   * <b>How it works</b>
   * </p>
   * <ul>
   * <li><b>Projection:</b> selects only {@code t.description} (not full
   * {@link Transaction} rows) for efficiency.</li>
   * <li><b>Validation filters:</b> excludes rows where {@code description} is
   * {@code null} or blank ({@code ''}).</li>
   * <li><b>Prefix matching (case-insensitive):</b>
   * uses {@code lower(t.description) LIKE lower(concat(:q, '%'))} so the query
   * {@code "sta"} becomes
   * {@code "sta%"} (matches anything that starts with {@code "sta"}).</li>
   * <li><b>Uniqueness:</b> {@code group by t.description} collapses repeated
   * descriptions so the same merchant
   * is returned once (e.g., "Starbucks" is not returned 50 times).</li>
   * <li><b>Ranking:</b> orders results by most common descriptions first
   * ({@code count(t) DESC}), then breaks ties
   * by the most recently used ({@code max(t.date) DESC}).</li>
   * <li><b>Limiting:</b> {@link Pageable} controls how many suggestions are
   * returned (acts like a SQL {@code LIMIT}).</li>
   * </ul>
   *
   * <p>
   * <b>Example</b>: {@code q="sta"} may match {@code "Starbucks"} and
   * {@code "Staples"}, but not {@code "Instacart"}.
   * </p>
   *
   * @param q        the user-entered query string used for prefix matching
   * @param pageable paging/limit information to cap the number of suggestions
   *                 returned
  ...
  /**
   * Returns the most recently used Transaction for the given description
   * (case-insensitive).
   *
   * <p>
   * Spring Data will sort matching transactions by {@code date DESC} and then
   * {@code id DESC}
   * (as a tiebreaker), and return only the first result (equivalent to
   * {@code LIMIT 1}).
   * </p>
   *
   * <p>
   * Note: This method does not filter out null/blank descriptions in the
   * database.
   * Input validation (trim, min length, non-blank) should be handled in the
   * service layer.
   * </p>
   *
   * @param description the transaction description to match (case-insensitive)
   * @return an Optional containing the most recent matching Transaction, or empty
   *         if none exist
   */
  ...
  @Query("""
        ...
        where t.category.id = :categoryId
          and c.type <> com.boojet.boot_api.domain.CategoryType.TRANSFER
      """)
  ...
  @Query("""
        ...
        from Transaction t
        join t.category c
        where c.type = com.boojet.boot_api.domain.CategoryType.INCOME
          and t.date between :start and :end
      """)
